...
* 落地协议支持 VLESS + REALITY + XTLS Vision 与 Shadowsocks 2022,
  两者都有用户级流量统计、额度限制与到期停用;
* V2 增加普通用户门户与三档节点访问等级;
* V2.1 增加节点可选 IPv6 订阅地址与节点级流量额度;
* V3 是一次整体 UI 改造:统一的设计 Token、状态三重编码、四态加载/空/错误处理,
  以及后台与门户两套区分开的界面语言;
* V4 第一块给节点加了第二种落地协议(Shadowsocks 2022),按节点选,
  端口与访问等级等设置对两种协议完全一样;
* V4 第二块新增「外部代理」:把机场订阅或朋友给的链接登记进面板,
  合并进用户的同一条订阅。可导入订阅源并定期同步,支持条目名前缀、
  访问等级与到期管理 —— 但**统计不到流量**,那些流量走的是上游的服务器。

## 当前状态

V1(基础能力)、V2(用户门户与访问等级)、V2.1(IPv6 与节点额度)与
V3(UI 改造)均已完成。
...
| Phase 9A~9B | 节点可选 IPv6 地址与订阅双条目 |
| Phase 9C~9D | 节点流量额度、周期统计与管理页面 |
| Phase 10A~10E | UI 改造:设计 Token、13 个公共组件、13 条路由全部重写、窄屏卡片化 |
...
* **V2.1 全部验收标准通过** —— [`docs/开发计划/V2/V2.1-验收报告.md`](docs/开发计划/V2/V2.1-验收报告.md)

日常运维见 [`docs/运维手册.md`](docs/运维手册.md),升级见
...
**节点可以配一个可选的公网 IPv6,它只影响订阅。** 填上之后这台机器在订阅里
出现两条 —— `洛杉矶 01` 与 `洛杉矶 01-IPV6` —— 用完全相同的 UUID、公网端口、
REALITY 公钥、short ID 与握手目标,只有名字和服务器地址不同。

它们指向的是**同一个 sing-box 入站**,所以数据库里也只有一条节点记录:
IPv6 是订阅生成时的逻辑展开,不是第二行。拆成两行会带来第二份配置版本、
第二串部署记录与第二套资源采样,而机器只有一台。

**SSH、探测、安装、部署、重启、流量同步与资源采集一律走 IPv4。**
双栈里只留一条管理通道,不然"节点连不上"会出现两种互相矛盾的结论。
因此改 IPv6 既不会断开连接池里的长连接,也不需要重新部署 —— 保存即生效。
不支持只有 IPv6 的节点。

**节点流量额度**可以设成不限量或固定值,周期支持"不重置"(统计创建以来的
累计流量)与"每月"(重置日 1~31,当月没有该日时落到月末)。边界统一取
UTC 00:00,页面上都标了出来。

额度**只用于统计与预警**:80% 黄、95% 红、100% 标"已超额",但不会停掉
sing-box、不会禁用节点、不会把节点从订阅里摘掉。流量同步有时间间隔,
各家 VPS 的计量口径也不一致,自动关掉一个共享节点会同时打断全部用户。

"重置"只是换统计起点:周期用量按 `traffic_ledger` 的时间范围现算,
历史流量、每日汇总、用户累计用量、计数器基线一行都不删,改回去也能还原。
周期边界只有一个实现(`traffic.CalculateNodePeriod`),列表、详情、仪表盘
与前端全部取它算出的结果。
...
Phase 9 交付:节点可选 IPv6 地址、订阅的 IPv4/IPv6 双条目展开、
节点流量额度与重置周期、周期用量统计与告警等级、节点列表与详情的周期流量展示。
...
* [升级说明](docs/升级说明.md) —— V1→V2、V2→V2.1 各会发生什么、要做什么、怎么回滚

面向开发:

* [API 说明](docs/API说明.md) —— 三套接口的完整清单与不可动摇的几条规则
* [数据库说明](docs/数据库说明.md) —— 迁移清单、核心表、升级兼容
* [V2.1 验收报告](docs/开发计划/V2/V2.1-验收报告.md) —— 18 条验收标准的逐条核对
...
* [V2.1 开发计划](docs/开发计划/V2/v2.1版本开发计划.md)
