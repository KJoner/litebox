...
**面板首页就是用户门户**,管理后台在 `/login`。首页留给用户是刻意的 ——
用户有 10 个、管理员只有 1 个,谁更多谁就该占默认路径;管理员加个书签就行,
用户拿到的是一个干净的域名。两套认证完全独立:表、Cookie、中间件都不共享,
同一个浏览器里可以同时登着两边。用户在里面能看到自己的流量、额度、到期时间、可用节点、
