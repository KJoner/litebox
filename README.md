...
- **外部代理**:机场订阅或朋友给的链接也能登记进来,合并进用户的同一条订阅
  (Shadowsocks / VMess / VLESS / Trojan / Hysteria2 / TUIC;前四种还能拿来
  当自建入口的出口,后两种走 QUIC —— 节点的精简构建拨不动它们,只能直连)

**运维**
- **服务巡检**:定期检查每台机器上的 sing-box 与 nginx 还在不在跑,
  挂了先拉起、拉不起来再重新下发配置(只在服务确实没跑时动手)
- **消息推送**:Bark 与 Telegram,推送节点服务停止、自动恢复结果、
  部署失败、流量额度告警
- **配置不落盘**:节点上的 sing-box 配置与备份可以只放在内存里,
  磁盘镜像、快照、退役硬盘上都拿不到(代价:机器重启后要靠巡检重新下发)
