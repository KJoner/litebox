...
type ChainProbe struct {
	...
	Landing string
	// Server / Port 是链式出站实际连的地址与公网端口,与配置里那一份同源。
	Server string
	Port   int
	// Code 是这条链路在落地上的用户代码(chain_000003),
	// 落地是外部代理时为空 —— 那是别人的机器,我们不往上面写任何东西。
	//
	// **它是这一步排查里唯一真正有用的东西**:落地的 sing-box 日志里
	// 按它就能挑出这条链路的行,而那正是判断「是这台坏了还是落地坏了」
	// 需要的材料。V7 给 nginx 透传定下的「失败必须带回落地那一侧的证据」
	// 在这条路上一直是空的 —— 面板拿不到落地的日志(节点锁不可重入,
	// 部署事务内不能再去连另一台机器),所以至少要把搜什么告诉人。
	Code string
	// LandingSSHPort 是落地那台机器的 sshd 端口,只用来判断二分诊断的
	// 结论成不成立。外部代理落地时为 0。
	LandingSSHPort int
}
