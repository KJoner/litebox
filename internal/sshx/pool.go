...
// ErrRemoteFailure 标记「失败出在远端的行为上,不是面板到节点这条 SSH 连接坏了」。
//
// Do 在传输层错误上会重连并【重跑整个 fn】。那对幂等的读操作是对的,
// 对已经动过节点的事务是灾难:部署走到回滚说明配置已经换过、服务已经重启过,
// 重跑一遍会再写一次配置、再重启一次服务,把这台机器上全部在线连接又踢一次。
// 更糟的是拨测会再打一次,而 OpenSSH 的 PerSourcePenalties 按次数累积 ——
// 第二次因此比第一次更容易失败,于是管理员看到的是第二轮的失败原因。
//
// **光靠字符串判不出来**:拨测失败的错误里几乎一定带 "EOF"(隧道通了、
// 对端在 SSH 握手时直接断开),而它与「这条连接坏了」长得一模一样。
// 真机上撞到过:一次链式部署因此整整跑了两轮、35.997 秒,服务重启两次。
var ErrRemoteFailure = errors.New("远端行为导致的失败")

// RemoteFailure 给错误打上「别重跑」的标记,消息文本一个字不变。
func RemoteFailure(err error) error {
	if err == nil {
		return nil
	}
	return remoteFailure{err: err}
}
...
// Unwrap 返回两个:原错误照常可以 errors.Is/As,同时带上这个标记。
func (e remoteFailure) Unwrap() []error { return []error{e.err, ErrRemoteFailure} }
...
func isConnectionError(err error) bool {
	...
	// 必须排在字符串匹配【之前】:远端行为导致的失败也带 "EOF",
	// 而它恰恰是最不能重跑的那一种 —— 那时节点已经被动过了。
